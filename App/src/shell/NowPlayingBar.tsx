...
/** Desktop-only trigger-based EQ button — owns its own popover state via the
 *  uncontrolled Modal (no open/onOpenChange passed). Mobile uses a separate,
 *  externally-controlled EqualizerPanel instance — see MoreMenu/NowPlayingBar. */
 ...
 * Everything else moves here. CRITICAL: this popover only ever holds
 * self-contained controls (repeat, AB-loop, tuning, lyrics toggle, queue,
 * volume) whose own state lives outside this component (zustand stores).
 * The EQ button does NOT render <EqualizerPanel> in here — it only calls
 * onOpenEq() and closes itself. EqualizerPanel itself is mounted once, by
 * NowPlayingBar, completely outside this popover — so closing this menu can
 * never unmount it (that was the bug: EQ used to live inside this popover's
 * tree and vanished the instant the menu closed). */
...
const MoreMenu = React.memo(({ onOpenEq }: { onOpenEq: () => void }) => {
  const { t } = useTranslation();
        ...
        >
          ...
          <div className="relative space-y-1">
            ...
            <MoreMenuRow>
              ...
              <div className="flex items-center gap-0.5">
                <TuningBtn />
                {/* EQ button: closes this menu and asks the parent to open
                    the externally-mounted EqualizerPanel. Does NOT render
                    EqualizerPanel itself — see the comment above MoreMenu. */}
                <button
                  type="button"
                  onClick={() => {
                    setOpen(false);
...
export const NowPlayingBar = React.memo(
  ({ onQueueToggle, queueOpen }: { onQueueToggle: () => void; queueOpen: boolean }) => {
    ...
    // Lives here — at the top of NowPlayingBar, which never unmounts while a
    // track is loaded — NOT inside MoreMenu's Popover.Content. That's the fix:
    // closing the "more" menu no longer tears down the EQ panel with it.
    ...
    return (
      <div className="npb">
          ...
          <div className="npb-content">
            ...
            <div className="npb-row npb-row-mobile">
              ...
              <div className="flex items-center gap-0.5 ml-auto">
                ...
                <PlayPauseBtn />
                <NextBtn />
                <MoreMenu onOpenEq={() => setEqOpen(true)} />
              </div>
            </div>
        ...
        {/* Mounted once, directly under NowPlayingBar — outside MoreMenu's
            popover tree entirely, so it survives that popover opening/closing.
            No trigger (no `children`): open state is fully external. */}
        <EqualizerPanel open={eqOpen} onOpenChange={setEqOpen} />
      </div>
    );
  },
);